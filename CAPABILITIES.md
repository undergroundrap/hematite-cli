...
- **Lead Computational Researcher Mandate**: Hematite detects when a query requires precise numeric or structural results (symbolic algebra, physics with unit safety, Big-O complexity audits, financial math, statistics) and automatically nudges the model to use `scientific_compute` instead of guessing from training data.
