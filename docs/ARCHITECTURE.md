...
SysAdmin and Network Admin inspection (`inspect_host` tool). 83+ read-only topics covering the full OS stack. Recent additions include `docker_filesystems` and `wsl_filesystems` for mount and storage truth. See CLAUDE.md for the complete topic reference.
