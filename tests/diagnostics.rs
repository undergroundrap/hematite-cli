...
#[tokio::test]
async fn test_base_tools_encode_all() {
    use hematite::tools::base_tools;
    let args = serde_json::json!({ "input": "hello" });
    let result = base_tools::execute(&args).await.unwrap();
    assert!(
        result.contains("encode"),
        "output should contain encode header"
    );
    assert!(result.contains("Base16"), "should show Base16");
    assert!(result.contains("Base32"), "should show Base32");
    assert!(result.contains("Base58"), "should show Base58");
    assert!(result.contains("Base85"), "should show Base85");
}

#[tokio::test]
async fn test_base_tools_encode_base32() {
    use hematite::tools::base_tools;
    let args = serde_json::json!({ "input": "hello", "encoding": "base32" });
    let result = base_tools::execute(&args).await.unwrap();
    // base32("hello") = NBSWY3DPEB3W64TMMQ (without padding)
    assert!(
        result.contains("NBSWY") || result.contains("Base32"),
        "base32 encode should include NBSWY prefix: {}",
        result
    );
}

#[tokio::test]
async fn test_base_tools_decode_base32_roundtrip() {
    use hematite::tools::base_tools;
    let encode_args = serde_json::json!({ "input": "roundtrip", "encoding": "base32" });
    let encoded = base_tools::execute(&encode_args).await.unwrap();
    // Extract the encoded value from output
    let encoded_val = encoded
        .lines()
        .find(|l| l.contains("Base32:"))
        .and_then(|l| l.split(':').nth(1))
        .map(|s| s.trim().to_string())
        .unwrap_or_default();

    let decode_args = serde_json::json!({
        "action": "decode",
        "input": encoded_val,
        "encoding": "base32"
    });
    let decoded = base_tools::execute(&decode_args).await.unwrap();
    assert!(
        decoded.contains("roundtrip"),
...
#[tokio::test]
async fn test_base_tools_decode_base58() {
    use hematite::tools::base_tools;
    // Encode then decode
    let enc = serde_json::json!({ "input": "test123", "encoding": "base58" });
    let encoded = base_tools::execute(&enc).await.unwrap();
    let encoded_val = encoded
        .lines()
        .find(|l| l.contains("Base58:"))
        .and_then(|l| l.split(':').nth(1))
        .map(|s| s.trim().to_string())
        .unwrap_or_default();
    ...
    let decoded = base_tools::execute(&dec).await.unwrap();
    assert!(
        decoded.contains("test123"),
        "base58 roundtrip should recover original: {}",
        decoded
    );
}

#[tokio::test]
async fn test_base_tools_decode_base16() {
    use hematite::tools::base_tools;
    ...
    assert!(
        ...
        "base16 decode should recover 'hello': {}",
        result
    );
}

#[tokio::test]
async fn test_base_tools_identify() {
    use hematite::tools::base_tools;
    // NBSWY3DP is base32 (hello)
    let args = serde_json::json!({ "action": "identify", "input": "NBSWY3DP" });
    let result = base_tools::execute(&args).await.unwrap();
    assert!(
        result.contains("Base32") || result.contains("identify"),
        "identify should detect base32: {}",
        result
    );
}

#[test]
fn test_routing_detects_base_tools() {
    use hematite::agent::routing::needs_base_tools;
    assert!(
        needs_base_tools("encode this as base32"),
        "should detect base32"
    );
    assert!(
        needs_base_tools("decode from base58"),
        "should detect base58"
    );
    assert!(needs_base_tools("z85 encode"), "should detect z85");
    assert!(needs_base_tools("base85 decode"), "should detect base85");
    assert!(
        !needs_base_tools("encode this as base64"),
        "base64 alone should not trigger base_tools"
    );
}

// ── lock_file_tools tests ─────────────────────────────────────────────────────

const CARGO_LOCK_SAMPLE: &str = r#"# This file is automatically @generated by Cargo.
...
#[tokio::test]
async fn test_lock_file_tools_cargo_info() {
    use hematite::tools::lock_file_tools;
    let args = serde_json::json!({ "text": CARGO_LOCK_SAMPLE, "format": "cargo" });
    let result = lock_file_tools::execute(&args).await.unwrap();
    assert!(result.contains("info"), "default action should be info");
    assert!(
        result.contains("Cargo.lock") || result.contains("cargo"),
        "should identify format"
    );
    assert!(
        result.contains("5") || result.contains("Packages"),
        "should show package count"
    );
}

#[tokio::test]
async fn test_lock_file_tools_cargo_duplicates() {
    use hematite::tools::lock_file_tools;
    let args = serde_json::json!({
        "action": "duplicates",
        "text": CARGO_LOCK_SAMPLE,
        "format": "cargo"
    });
    let result = lock_file_tools::execute(&args).await.unwrap();
    assert!(
        result.contains("tokio"),
        "duplicates should flag tokio at two versions: {}",
        result
    );
    assert!(
        result.contains("1.35.0") || result.contains("0.2.25"),
        "should show both versions"
    );
}

#[tokio::test]
async fn test_lock_file_tools_cargo_list() {
    use hematite::tools::lock_file_tools;
    let args = serde_json::json!({
        "action": "list",
        "text": CARGO_LOCK_SAMPLE,
    ...
    let result = lock_file_tools::execute(&args).await.unwrap();
    assert!(result.contains("serde"), "list should include serde");
    assert!(result.contains("tokio"), "list should include tokio");
    assert!(result.contains("my-app"), "list should include local crate");
}

#[tokio::test]
async fn test_lock_file_tools_cargo_search() {
    use hematite::tools::lock_file_tools;
    let args = serde_json::json!({
        "action": "search",
        "text": CARGO_LOCK_SAMPLE,
    ...
    assert!(
        ...
        "search should find serde packages"
    );
    assert!(
        result.contains("serde_json") || result.contains("1.0"),
        "should include serde_json"
    );
}

#[tokio::test]
async fn test_lock_file_tools_npm_info() {
    use hematite::tools::lock_file_tools;
    let args = serde_json::json!({ "text": NPM_LOCK_SAMPLE, "format": "npm" });
    let result = lock_file_tools::execute(&args).await.unwrap();
    assert!(
        result.contains("package-lock.json") || result.contains("npm"),
        "should identify npm format"
    );
    assert!(
        result.contains("lodash") || result.contains("2") || result.contains("Packages"),
        "should show packages"
    );
}

#[tokio::test]
async fn test_lock_file_tools_npm_list() {
    use hematite::tools::lock_file_tools;
    let args = serde_json::json!({
        "action": "list",
        "text": NPM_LOCK_SAMPLE,
        "format": "npm"
    });
    let result = lock_file_tools::execute(&args).await.unwrap();
    assert!(result.contains("lodash"), "list should show lodash");
    assert!(result.contains("express"), "list should show express");
}

#[test]
fn test_routing_detects_lock_file_tools() {
    use hematite::agent::routing::needs_lock_file_tools;
    assert!(
        needs_lock_file_tools("analyze the Cargo.lock file"),
        "should detect Cargo.lock"
    );
    assert!(
        needs_lock_file_tools("check package-lock.json for duplicates"),
        "should detect package-lock.json"
    );
    assert!(
        needs_lock_file_tools("find duplicate packages in yarn.lock"),
        "should detect yarn.lock"
    );
    assert!(
        needs_lock_file_tools("parse the lockfile"),
        "should detect lockfile"
    );
    assert!(
        !needs_lock_file_tools("read the config file"),
