...
- **Shared HTML template**: `src/agent/html_template.rs` owns the CSS, JS, and `build_html_shell(title, version, content_html)` function — any feature that saves an HTML file should call this instead of building its own document from scratch; `he()` HTML escaping is also exported from here
- `/copy`: copy the session transcript manually
