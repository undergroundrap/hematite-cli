...
- `hematite --fix "<issue>"`: targeted fix plan � no model required; describe the problem in plain English and Hematite keyword-matches it to the right `inspect_host` topics, runs them, filters fix recipes against the real output, and saves a step-by-step fix plan to `.hematite/reports/fix-DATE.md`. Add `--report-format html --open` for a double-clickable HTML report. Examples: `hematite --fix "PC running slow"`, `hematite --fix "can't connect to internet" --report-format html --open`, `hematite --fix "BSOD after update"`. Covers ~90% of common IT problems via pure keyword matching � no model, no cloud, no LM Studio required.
...
- `hematite --inventory`: lists all available inspect_host topics categorized by domain (System, Network, Identity, Developer). No model, no TUI. Pipe to grep to find a topic.
- `hematite --inspect <topic[,topic2,...]>`: runs any inspect_host topic directly to stdout. No model, no TUI. Comma-separate for multi-topic runs. Add `--open` to save and launch the output file. Example: `hematite --inspect wifi,latency,dns_cache`
- `hematite --query "<text>"`: natural-language query routed to the right inspect_host topics via keyword detection. No model, no TUI. Example: `hematite --query "why is my PC slow"`
...
- `hematite --watch <topic> --alert <pattern>`: alert mode — silent heartbeat when output does not contain the pattern; rings bell and prints full output when it does. Case-insensitive. Example: `hematite --watch thermal --alert throttl`
- `hematite --diff <topic[,topic2,...]>`: takes two snapshots separated by `--diff-after` seconds (default 30) and shows a colored unified diff. Useful for before/after diagnosis. Example: `hematite --diff processes --diff-after 60`
...
- `hematite --snapshots`: lists all saved snapshots in `.hematite/snapshots/` with name, size, and age. Example: `hematite --snapshots`
- `--open`: works with `--report`, `--diagnose`, `--triage`, `--fix`, and `--inspect` � saves the output file and opens it in the default application (browser for HTML, editor for Markdown); `hematite --diagnose --report-format html --open` is the one-shot USB-drive path: runs full staged triage, saves an HTML report, and opens it in the browser immediately
...
- **New fix recipes** � ? Done. Added: service start failure, RDP disabled/unreachable, Windows Update service broken, and PrintNightmare not mitigated. Total: 29 recipes.
