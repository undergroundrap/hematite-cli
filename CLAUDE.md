...
- `--report-format <fmt>`: output format for all report commands. `md` (default), `json`, or `html`. JSON produces structured output with health grade, action items, and per-section data. All headless commands (`--triage`, `--diagnose`, `--fix`, `--fix-all`, `--report`, `--inspect`, `--query`, `--diff`, `--compare`, `--snapshots`, `--inventory`) support JSON format for scripting and CI integration.
