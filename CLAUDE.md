...
It features a high-fidelity integrated host inspection suite covering **81+ read-only diagnostic topics** for precision triage.
...
- **Deep WSL / Docker filesystem auditing** — high operator value because this is where real developer workstation breakage hides: wrong bind mounts, missing volume paths, Docker Desktop disk bloat, WSL distro storage growth, bad working directories, and mount confusion between Windows and Linux paths. Implement as new `inspect_host` topics focused on storage and mount truth rather than raw runtime state. Minimum useful scope: Docker bind mounts + named volumes + per-container mount summary, WSL distro VHDX sizing + distro disk usage hints + Windows/WSL path bridge checks. The response format should always be `finding -> impact -> exact fix steps`, not a raw dump. Add routing phrases, HOST INSPECTION MODE bullets, README/CAPABILITIES/CLAUDE updates, and diagnostics tests before calling it shipped.
- **Advanced LAN / UPnP / neighborhood inspection** — second priority because this compresses a lot of real helpdesk pain: "why can't this machine see that printer/NAS/PC", local discovery failures, share reachability, multicast noise, and consumer-network weirdness. Implement as explicit LAN inspection topics rather than hiding behind UDP listener trivia. Minimum useful scope: neighborhood discovery summary, SMB/NetBIOS visibility, mDNS/SSDP/UPnP listener surface, gateway/device-discovery hints, and a plain-English diagnosis path for "discovery broken vs service missing vs firewall blocked". Again, shape the output as concrete operator guidance rather than packet trivia.
- **Voltage telemetry for overclocker** — useful, but lower priority than the two support-heavy features above. Clocks, fans, power draw, and throttle reasons already ship; the real gap is trustworthy voltage reporting and clearer electrical-limit interpretation. Only mark this done once the harness can expose real voltage rails or confidently explain why they are unavailable on a given driver/GPU path. When added, wire it into `overclocker` without regressing the current NVIDIA clock/fan/power path, update docs to say voltage only when it is actually present, and keep the wording precise enough that operators do not mistake "power draw" for "voltage telemetry".
