...
- `hematite --compare <name1>,<name2>`: diff two saved snapshots against each other without a live run. Example: `hematite --compare before-update,after-update`
...
- `--report-format <fmt>`: output format for all report commands. `md` (default), `json`, or `html`. JSON produces structured output with health grade, action items, and per-section data. All headless commands (`--triage`, `--diagnose`, `--fix`, `--fix-all`, `--report`, `--inspect`, `--query`, `--diff`, `--snapshots`, `--inventory`) support JSON format for scripting and CI integration.
