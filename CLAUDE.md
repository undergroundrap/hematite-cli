...
- **Redirection Discipline**: Common diagnostic commands (e.g. `arp -a`, `Get-Process`, `tasklist`) are silently redirected to `inspect_host`. An infinite loop breaker is handled by the harness; do not retry if a redirection notice is received.
- **Telemetry Whitelisting**: `get-counter` and diagnostic read-only PowerShell commands are whitelisted in `guard.rs` to reduce operator approval friction.
