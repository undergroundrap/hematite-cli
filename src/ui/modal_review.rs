...
pub fn draw_diff_review(f: &mut Frame, review: &ActiveReview) {
    ...
    let before_para = Paragraph::new(review.before.clone())
        .block(Block::default().title(" Original Context ").borders(Borders::ALL).fg(Color::Red))
        .wrap(Wrap { trim: true });
    ...
    let cleaned_after = review.after
    ...
    let after_para = Paragraph::new(final_after)
        .block(Block::default().title(" AI Synthesized Patch ").borders(Borders::ALL).fg(Color::Cyan))
        .wrap(Wrap { trim: true });
    f.render_widget(after_para, chunks[2]);

    // Footer Triggers
    let footer = Paragraph::new(" [Y] Accept  |  [N] Abort  |  [R] Retry Synthesis ")
        .block(Block::default().borders(Borders::ALL).style(Style::default().add_modifier(Modifier::BOLD)))
        .alignment(ratatui::layout::Alignment::Center);
