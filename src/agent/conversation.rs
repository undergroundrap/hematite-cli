...
use crate::agent::routing::{
    ...
    needs_asn1_tools, needs_astro_tools, needs_audio_file_tools, needs_aws_tools,
    needs_babel_tools, needs_base_tools, needs_bencode_tools, needs_bin_pack_tools,
    ...
    needs_cmake_tools, needs_code_metrics, needs_color_tools, needs_compression_tools,
    needs_computation_sandbox, needs_conda_tools, needs_cors_tools, needs_coverage_tools,
    needs_crash_debug, needs_cron_tools, needs_csp_tools, needs_css_tools, needs_csv_tools,
    needs_curl_tools, needs_cvss_tools, needs_data_gen_tools, needs_date_tools,
    needs_dependency_audit, needs_dex_tools, needs_diff3_tools, needs_diff_tools, needs_dns_tools,
    needs_docker_compose_tools, needs_docker_ops, needs_dockerfile_tools, needs_dotenv_tools,
    needs_dotnet_tools, needs_duration_tools, needs_elf_tools, needs_em_tools, needs_email_tools,
    needs_encode_tools, needs_env_diff, needs_env_schema_tools, needs_epub_tools,
    needs_eslint_tools, needs_exif_tools, needs_file_tree_tools, needs_financial_tools,
    needs_find_tools, needs_font_tools, needs_format, needs_fraction_tools, needs_geo_tools,
    needs_geometry_tools, needs_git_log_tools, needs_github_actions_tools, needs_github_ops,
    needs_gitignore_tools, needs_gitlab_ci_tools, needs_glob_tools, needs_go_mod_tools,
    ...
    needs_ip_tools, needs_iptables_tools, needs_jest_tools, needs_journald_tools, needs_jq_tools,
    needs_json_patch_tools, needs_json_tools, needs_jsonl_tools, needs_jsonrpc_tools,
    needs_jsonschema_tools, needs_junit_tools, needs_jwk_tools, needs_jwt_tools, needs_k8s_tools,
    needs_keyval_tools, needs_latex_tools, needs_ldif_tools, needs_leb128_tools,
    ...
    needs_maven_tools, needs_mechanics_tools, needs_mermaid_tools, needs_mime_tools,
    needs_money_tools, needs_msgpack_tools, needs_multipart_tools, needs_music_tools,
    needs_nato_tools, needs_net_lookup_tools, needs_network_header_tools, needs_nginx_conf_tools,
    needs_nmap_tools, needs_notebook_tools, needs_nuclear_tools, needs_number_sequence_tools,
    ...
    needs_plist_tools, needs_port_check, needs_postman_tools, needs_prettier_tools,
    needs_printf_tools, needs_prometheus_tools, needs_proto_tools, needs_protobuf_wire_tools,
    needs_quantum_tools, needs_regex_tools, needs_relativity_tools, needs_requirements_tools,
    ...
    needs_strace_tools, needs_string_metric_tools, needs_stylelint_tools, needs_subnet_tools,
    needs_svg_tools, needs_systemd_tools, needs_table_tools, needs_tar_tools, needs_template_gen,
    ...
    needs_totp_tools, needs_trie_tools, needs_tsconfig_tools, needs_unicode_tools,
...
impl ConversationManager {
    ...
    pub async fn run_turn(
        ...
        if loop_intervention.is_none() && needs_maven_tools(&effective_user_input) {
            loop_intervention = Some(
                "MAVEN NOTICE: Use the `maven_tools` tool to parse, inspect, and validate Maven pom.xml files without external utilities. \
                 Actions: info (default — coordinates, packaging, parent, Java version, dep/plugin/profile counts), \
                 deps (dependency table with scope; optional 'scope' and 'filter' args), \
                 plugins (build plugins and pluginManagement), \
                 profiles (all profiles with activation type and counts), \
                 properties (<properties> key-value pairs), \
                 validate (missing coordinates, version ranges, SNAPSHOT in release, duplicates). \
                 Pass 'file' (path to pom.xml) or 'pom'/'xml' (inline). \
                 Example: maven_tools(action: 'info', file: 'pom.xml') or \
                 maven_tools(action: 'deps', file: 'pom.xml', scope: 'test') or \
                 maven_tools(action: 'validate', pom: '<project>...</project>')."
                    .to_string(),
            );
        }

        if loop_intervention.is_none() && needs_gradle_tools(&effective_user_input) {
            loop_intervention = Some(
                "GRADLE NOTICE: Use the `gradle_tools` tool to parse, inspect, and validate Gradle build files (build.gradle and build.gradle.kts) without external utilities. \
                 Actions: info (default — group/version, repositories, plugins, dependency config counts), \
                 deps (dependencies grouped by configuration; optional 'configuration' and 'filter' args), \
                 tasks (custom task definitions with type and dependsOn), \
                 plugins (applied plugins with version and apply flag), \
                 properties (ext/extra property assignments), \
                 validate (missing group/version, no repos, deprecated configs, jcenter, missing dep versions). \
                 Pass 'file' (path to build.gradle or build.gradle.kts) or 'gradle'/'text' (inline). \
        ...
        if loop_intervention.is_none() && needs_stylelint_tools(&effective_user_input) {
            loop_intervention = Some(
                "STYLELINT NOTICE: Use the `stylelint_tools` tool to parse, inspect, and validate Stylelint configuration files without external utilities. \
                 Actions: info (default — full config overview: rule/plugin/extends/override counts and severity summary), \
                 rules (detailed rule listing grouped by severity with options), \
                 plugins (numbered plugin list), \
                 extends (config inheritance chain with ordering note), \
                 validate (common issues: no rules/extends/plugins, deprecated v15+ rules, SCSS/order rules without matching plugins, unknown keys). \
                 Pass 'config' for inline JSON or YAML, or 'file' for a path to .stylelintrc, .stylelintrc.json, .stylelintrc.yaml, or package.json. \
                 Example: stylelint_tools(action: 'rules', file: '.stylelintrc.json')."
                    .to_string(),
            );
        }
        ...
        if loop_intervention.is_none() && needs_physics_tools(&effective_user_input) {
            loop_intervention = Some(
                "PHYSICS NOTICE: Use the `physics_tools` tool to look up physical constants and evaluate physics formulas without external utilities. \
                 Actions: constant (default — look up by name/symbol/keyword; e.g. 'c', 'planck', 'boltzmann'), \
                 formula (evaluate a named formula solving for the missing variable; pass 'name' and 'vars' object with known values — omit the unknown), \
                 list (browse all constants or formulas; pass what='constants' or what='formulas'; optional 'domain' filter), \
                 domains (show all available physics domains with counts). \
                 ~29 NIST constants and ~20 named formulas covering mechanics, electromagnetism, thermodynamics, optics, quantum, gravity, waves, and relativity. \
                 Example: physics_tools(action: 'constant', query: 'speed of light') or \
                 physics_tools(action: 'formula', name: 'kinetic_energy', vars: {m: 5, v: 10}) or \
                 physics_tools(action: 'formula', name: 'ideal_gas', vars: {n: 1, T: 273.15, V: 0.0224})."
                    .to_string(),
            );
        }

        if loop_intervention.is_none() && needs_chemistry_tools(&effective_user_input) {
            loop_intervention = Some(
                "CHEMISTRY NOTICE: Use the `chemistry_tools` tool for chemical equation balancing, stoichiometry, solution calculations, pH, and gas laws without external utilities. \
                 Actions: balance (default — balance a chemical equation using Gaussian elimination; pass 'equation' like 'H2 + O2 -> H2O'), \
                 stoichiometry (mole/mass calculations from a balanced equation; pass 'equation', 'reactant', 'product', and 'moles' or 'grams'), \
                 solution (molarity M=n/V and dilution C₁V₁=C₂V₂; pass moles_solute+volume_L or three of C1/V1/C2/V2), \
                 ph (pH conversions and Henderson-Hasselbalch buffer; pass 'pH_value' or 'Ka'+'acid_conc'+'base_conc'), \
                 gas (ideal gas law PV=nRT; pass three of P/V/n/T; use unit='L' for liters). \
        ...
        if loop_intervention.is_none() && needs_bio_tools(&effective_user_input) {
            loop_intervention = Some(
                "BIOINFORMATICS NOTICE: Use the `bio_tools` tool to analyze biological sequences without external utilities. \
                 Actions: info (default — sequence type detection, length, base composition, GC%; pass 'sequence'), \
                 complement (reverse complement of DNA), \
                 transcribe (DNA to mRNA or mRNA to DNA template), \
                 translate (mRNA/DNA to protein; 'frame': 1/2/3/-1/-2/-3; 'all_frames: true' for all 6), \
                 gc (GC content with sliding window analysis), \
                 orfs (find open reading frames in all 6 frames; 'min_length' in aa), \
                 codons (codon usage frequency table), \
                 parse_fasta (parse multi-record FASTA files; pass 'text' or 'file'). \
                 Example: bio_tools(action: 'translate', sequence: 'ATGAAACCC...') or \
                 bio_tools(action: 'parse_fasta', file: 'sequences.fasta')."
                    .to_string(),
            );
        }

        if loop_intervention.is_none() && needs_gpu_tools(&effective_user_input) {
            loop_intervention = Some(
                "GPU TOOLS NOTICE: Use the `gpu_tools` tool to estimate LLM VRAM requirements and look up GPU specifications without external utilities. \
                 Actions: estimate (default — VRAM needed for a model; pass 'params' in billions and 'quant': fp16/q8/q6/q5/q4/q3/q2), \
                 batch (max batch size given VRAM; pass 'params', 'quant', 'vram_gb'), \
                 info (GPU specs by model name; pass 'gpu' like 'RTX 4070'), \
                 parse (parse pasted nvidia-smi text output; pass 'text'), \
                 budget (VRAM budget planner showing which model sizes fit; pass 'vram_gb'). \
                 Example: gpu_tools(action: 'estimate', params: 7.0, quant: 'q4_k_m', context: 4096) or \
                 gpu_tools(action: 'info', gpu: 'RTX 4070') or \
                 gpu_tools(action: 'budget', vram_gb: 12.0)."
                    .to_string(),
            );
        }

        if loop_intervention.is_none() && needs_astro_tools(&effective_user_input) {
            loop_intervention = Some(
                "ASTRONOMY NOTICE: Use the `astro_tools` tool for astronomy calculations without external utilities. \
                 Actions: planet (default — heliocentric positions and magnitudes for all planets; optional 'body' to filter, 'date' YYYY-MM-DD), \
                 rise_set (rise/transit/set times; pass 'ra', 'dec' in degrees, 'lat', 'lon'), \
                 separation (angular separation between two sky objects; pass 'ra'/'dec' and 'ra2'/'dec2'), \
                 magnitude (magnitude scale reference or flux-to-magnitude conversion; pass 'mag1'/'mag2' or 'flux'), \
                 distance (AU/ly/pc/km/m conversions; pass 'value', 'from_unit', 'to_unit'), \
                 constellation (lookup by name or abbreviation; pass 'query'), \
                 moon_phase (current or dated Moon phase; pass optional 'date'), \
                 julian (Julian Date conversion; pass 'date' or 'jd'). \
                 Example: astro_tools(action: 'planet', date: '2025-01-01') or \
                 astro_tools(action: 'separation', ra: 83.82, dec: -5.39, ra2: 101.29, dec2: -16.72) or \
        ...
        if loop_intervention.is_none() && needs_signal_tools(&effective_user_input) {
            loop_intervention = Some(
                "SIGNAL PROCESSING NOTICE: Use the `signal_tools` tool for DSP calculations without external utilities. \
                 Actions: dft (default — Discrete Fourier Transform with magnitude/phase per bin; pass 'samples' array, optional 'sample_rate'), \
                 idft (Inverse DFT from 'real' and 'imag' arrays), \
                 convolve (1D convolution of 'samples' with 'kernel'), \
                 fir (windowed-sinc FIR filter design; pass 'cutoff', optional 'taps', 'filter_type': lowpass/highpass/bandpass/bandstop, 'window_type': hamming/hanning/blackman/kaiser), \
                 window (generate a window function; pass 'window_type', 'length'), \
                 stats (signal statistics: mean/RMS/variance/crest factor/zero-crossings/entropy; pass 'samples'), \
                 resample (polyphase resampling; pass 'samples', 'up', 'down'), \
                 autocorr (autocorrelation function; pass 'samples', optional 'sample_rate'). \
        ...
        if loop_intervention.is_none() && needs_cors_tools(&effective_user_input) {
            loop_intervention = Some(
                "CORS NOTICE: Use the `cors_tools` tool to parse, validate, generate, and simulate CORS headers without external utilities. \
                 Actions: parse (default — decode all Access-Control-* headers with notes; pass 'headers' object), \
                 validate (check headers for spec violations — wildcard+credentials conflict, invalid methods, max-age overflow; pass 'headers'), \
                 generate (build response headers for a requesting 'origin' given 'allowed_origins'/'allowed_methods'/'allowed_headers'/'allow_credentials'/'max_age'), \
                 explain (plain-English meaning of each CORS header; pass 'headers'), \
        ...
        if loop_intervention.is_none() && needs_web_manifest_tools(&effective_user_input) {
            loop_intervention = Some(
                "WEB MANIFEST NOTICE: Use the `web_manifest_tools` tool to parse, validate, and inspect PWA Web App Manifest files without external utilities. \
                 Actions: parse (default — full manifest summary; pass 'manifest' as JSON object/string or 'file' path), \
                 validate (check installability requirements — name, icons with 192×512 sizes, maskable icon, display mode, start_url; VALID/INVALID verdict), \
                 icons (tabular listing of all icon entries with src, sizes, type, and purpose), \
                 screenshots (list screenshots with sizes, form_factor, and label), \
                 info (concise install/display summary — display mode, orientation, scope, theme/background colors, advanced API declarations, shortcuts). \
                 Example: web_manifest_tools(manifest: {\"name\":\"My App\",\"start_url\":\"/\",\"display\":\"standalone\"}) or \
        ...
        if loop_intervention.is_none() && needs_babel_tools(&effective_user_input) {
            loop_intervention = Some(
                "BABEL NOTICE: Use the `babel_tools` tool to parse, inspect, and validate Babel configuration files without external utilities. \
                 Actions: info (default — full config overview: preset/plugin/env/override counts and tables), \
                 presets (numbered preset listing with options), \
                 plugins (numbered plugin listing with options), \
                 env (environment-specific configuration: production/development/test blocks), \
                 validate (common issues: @babel/preset-env without targets, deprecated stage-x/es2015/react presets, duplicate presets/plugins). \
                 Pass 'config' for inline JSON or YAML, or 'file' for a path to babel.config.json, .babelrc, .babelrc.json, .babelrc.yaml, or package.json. \
                 Example: babel_tools(action: 'validate', file: 'babel.config.json')."
                    .to_string(),
            );
        }
        ...
        for _iter in 0..max_iters {
            ...
            if let Some(calls) = tool_calls {
                ...
                if implement_current_plan
                    && !implementation_started
                    && self.recovery_context.consume_implement_plan_nudge()
                {
                    loop_intervention = Some(
