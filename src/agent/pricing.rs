...
pub fn get_pricing(model: &str) -> ModelPricing {
    ...
    if m.contains("gemma-4") || m.contains("gemini-1.5") {
        return ModelPricing { input: 0.15, output: 0.60 };
    }
    ...
    if m.contains("haiku") || m.contains("mini") || m.contains("7b") || m.contains("8b") || m.contains("9b") || m.contains("12b") || m.contains("14b") {
        return ModelPricing { input: 0.25, output: 1.25 };
    }
    
    // Default safe fallback (standard Haiku-like rate)
    ModelPricing { input: 0.25, output: 1.25 }
}
