...
impl LspClient {
    pub fn spawn(command: &str, args: &[String]) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if line.starts_with("Content-Length: ") {
                    ...
                    if let Err(_) = reader.read_exact(&mut body).await { break; }
                        ...
                        if let Some(id) = json_body.get("id").and_then(|v| v.as_u64()) {
                            ...
                            if let Some(tx) = pending.remove(&id) {
                                if let Some(err) = json_body.get("error") {
                                    ...
                                    let result = json_body.get("result").cloned().unwrap_or(Value::Null);
                                    let _ = tx.send(Ok(result));
                                }
                            }
                        } else if let Some(method) = json_body.get("method").and_then(|v| v.as_str()) {
                            // This is a notification
                            if method == "textDocument/publishDiagnostics" {
                                if let Some(params) = json_body.get("params") {
                                    if let Some(uri) = params.get("uri").and_then(|v| v.as_str()) {
                                        let mut diags = diagnostics_clone.lock().await;
                                        diags.insert(uri.to_string(), params.get("diagnostics").cloned().unwrap_or(Value::Null));
                                    }
                                }
                            }
                        }
    ...
    pub async fn call(&self, method: &str, params: Value) -> Result<Value, String> {
        ...
        rx.await.map_err(|_| "LSP Response Channel Closed".to_string())?
    }
