...
function copyReport() {{
  ...
  var sections = document.querySelectorAll('pre');
  var lines = ['{title} — {hostname}', 'Health Score: {grade} — {label}', '{summary}', ''];
  ...
  sections.forEach(function(pre) {{
    var lbl = pre.closest('details');
    if(lbl) {{ var s = lbl.querySelector('summary'); if(s) lines.push('\n[' + s.innerText.trim() + ']'); }}
    lines.push(pre.innerText.trim());
  }});
  navigator.clipboard.writeText(lines.join('\n')).then(function() {{
    btn.textContent = 'Copied!';
    btn.classList.add('copied');
    setTimeout(function() {{ btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg> Copy report for AI'; btn.classList.remove('copied'); }}, 2000);
  }});
}}
</script>
</body>
