...
fn parse_wav(b: &[u8]) -> Option<WavMeta> {
    ...
    if &b[0..4] != b"RIFF" || &b[8..12] != b"WAVE" { return None; }
    ...
    while pos + 8 <= b.len() {
        ...
        if tag == b"fmt " { fmt_off = pos + 8; }
        if tag == b"data" { data_size = chunk_size as u32; }
        pos += 8 + chunk_size + (chunk_size & 1);
        if pos >= b.len() { break; }
    }
    if fmt_off + 16 > b.len() { return None; }
    Some(WavMeta {
...
fn dispatch_wav(action: &str, b: &[u8]) -> String {
    ...
    let duration_secs = if m.byte_rate > 0 { m.data_size as f64 / m.byte_rate as f64 } else { 0.0 };
    match action {
        "info" | "metadata" => {
            let mut out = vec![
                "Format:          WAV / RIFF".to_string(),
                format!("Audio encoding:  {} (code {})", format_name(m.audio_format), m.audio_format),
                format!("Channels:        {}", m.channels),
                format!("Sample rate:     {} Hz", m.sample_rate),
                format!("Bit depth:       {}-bit", m.bits_per_sample),
                format!("Byte rate:       {} B/s ({:.0} kbps)", m.byte_rate, m.byte_rate as f64 * 8.0 / 1000.0),
                format!("Data size:       {} ({})", m.data_size, human_size(m.data_size as u64)),
                format!("Duration:        {}", human_dur(duration_secs)),
                format!("File size:       {}", human_size(m.file_size)),
            ];
            if m.channels == 1 { out.push("Channel layout:  Mono".to_string()); }
            else if m.channels == 2 { out.push("Channel layout:  Stereo".to_string()); }
            else { out.push(format!("Channel layout:  {} channels", m.channels)); }
            out.join("\n")
        }
        "duration" => format!("Duration: {} ({:.3} s)", human_dur(duration_secs), duration_secs),
        "validate" => {
            let mut issues = vec![];
            if m.audio_format != 1 && m.audio_format != 3 && m.audio_format != 65534 {
                issues.push(format!("Non-PCM encoding: code {}", m.audio_format));
            }
            if m.channels == 0 { issues.push("Invalid channel count: 0".to_string()); }
            if m.sample_rate == 0 { issues.push("Invalid sample rate: 0".to_string()); }
            if m.bits_per_sample == 0 { issues.push("Invalid bit depth: 0".to_string()); }
            if m.data_size == 0 { issues.push("Empty data chunk".to_string()); }
            let expected_br = (m.sample_rate as u32) * (m.channels as u32) * (m.bits_per_sample as u32 / 8);
            if m.audio_format == 1 && expected_br != m.byte_rate {
                issues.push(format!("Byte rate mismatch: header={} expected={}", m.byte_rate, expected_br));
            }
            if issues.is_empty() { "VALID — WAV file structure is well-formed.".to_string() }
            else { format!("WARNINGS:\n{}", issues.iter().map(|s| format!("  • {}", s)).collect::<Vec<_>>().join("\n")) }
        }
...
fn parse_id3v2(b: &[u8]) -> Option<Id3v2Tag> {
    ...
    while pos + 10 <= tag_end {
        ...
        pos += 10;
        if fsize == 0 || pos + fsize > tag_end { break; }
        ...
        if fid.starts_with('T') || fid == "COMM" || fid == "USLT" {
            ...
            if !text.is_empty() { frames.push((fid.clone(), text)); }
        }
        pos += fsize;
    }
    Some(Id3v2Tag { version, revision, flags, size, frames })
}

fn decode_id3_text(b: &[u8], encoding: u8) -> String {
    match encoding {
        0 => b.iter().take_while(|&&c| c != 0).map(|&c| c as char).collect(),
        1 | 2 => {
            ...
            let pairs: Vec<u16> = b[start..]
                ...
                .map(|c| if bom_be { u16::from_be_bytes([c[0], c[1]]) } else { u16::from_le_bytes([c[0], c[1]]) })
                .collect();
            String::from_utf16_lossy(&pairs)
        }
        3 => String::from_utf8_lossy(b.iter().take_while(|&&c| c != 0).cloned().collect::<Vec<_>>().as_slice()).into_owned(),
        _ => String::new(),
    }
}

fn read_id3v1(b: &[u8]) -> Vec<(String, String)> {
    if b.len() < 128 { return vec![]; }
    let start = b.len() - 128;
    if &b[start..start + 3] != b"TAG" { return vec![]; }
    let latin1 = |slice: &[u8]| -> String {
        slice.iter().take_while(|&&c| c != 0).map(|&c| c as char).collect::<String>().trim().to_string()
    };
    let mut tags = vec![];
    let t = latin1(&b[start + 3..start + 33]); if !t.is_empty() { tags.push(("TIT2".to_string(), t)); }
    let a = latin1(&b[start + 33..start + 63]); if !a.is_empty() { tags.push(("TPE1".to_string(), a)); }
    let al = latin1(&b[start + 63..start + 93]); if !al.is_empty() { tags.push(("TALB".to_string(), al)); }
    let y = latin1(&b[start + 93..start + 97]); if !y.is_empty() { tags.push(("TDRC".to_string(), y)); }
    tags
}
...
const MP3_BITRATES: [[u32; 16]; 5] = [
    ...
    [0, 8,16,24, 32, 40, 48, 56, 64, 80, 96,112,128,144,160,0], // MPEG2 L2/L3
];
...
struct Mp3Frame { bitrate: u32, sample_rate: u32, channels: u8, layer: u8, mpeg_ver: u8 }

fn find_mp3_frame(b: &[u8], start: usize) -> Option<Mp3Frame> {
    let mut i = start;
    while i + 4 <= b.len() {
        if b[i] != 0xFF || (b[i + 1] & 0xE0) != 0xE0 { i += 1; continue; }
        let h = ((b[i] as u32) << 24) | ((b[i + 1] as u32) << 16) | ((b[i + 2] as u32) << 8) | (b[i + 3] as u32);
        let mpeg_idx = ((h >> 19) & 3) as usize;
        let layer_idx = ((h >> 17) & 3) as usize;
        let br_idx = ((h >> 12) & 0xF) as usize;
        let sr_idx = ((h >> 10) & 3) as usize;
        let ch = ((h >> 6) & 3) as u8;
        if mpeg_idx == 1 || layer_idx == 0 || br_idx == 0 || br_idx == 15 || sr_idx == 3 { i += 1; continue; }
        let mpeg_ver: u8 = match mpeg_idx { 3 => 1, 2 => 2, 0 => 25, _ => 0 };
        let layer: u8 = 4u8.saturating_sub(layer_idx as u8);
        let row = if mpeg_ver == 1 { layer as usize - 1 } else if layer == 1 { 3 } else { 4 };
        let bitrate = MP3_BITRATES[row][br_idx] * 1000;
        let sr_row = if mpeg_ver == 1 { 0 } else if mpeg_ver == 2 { 1 } else { 2 };
        let sample_rate = MP3_SAMPLERATES[sr_row][sr_idx];
        if bitrate == 0 || sample_rate == 0 { i += 1; continue; }
        let channels = if ch == 3 { 1 } else { 2 };
        return Some(Mp3Frame { bitrate, sample_rate, channels, layer, mpeg_ver });
    }
    None
}

fn dispatch_mp3(action: &str, b: &[u8]) -> String {
    if b.len() < 4 { return "Error: file too small.".to_string(); }
    let has_id3v2 = &b[0..3] == b"ID3";
    let id3v2 = if has_id3v2 { parse_id3v2(b) } else { None };
    let id3v1 = read_id3v1(b);
    let tags: Vec<(String, String)> = id3v2.as_ref().map(|t| t.frames.clone()).unwrap_or_default()
        .into_iter()
        .chain(if id3v2.is_none() { id3v1.clone() } else { vec![] })
        .collect();
    ...
    let tag_label = |k: &str| -> String {
        match k { "TIT2" => "Title".into(), "TPE1" => "Artist".into(), "TALB" => "Album".into(),
            "TDRC" | "TYER" => "Year".into(), "TRCK" => "Track".into(), "TCON" => "Genre".into(),
            "TCOM" => "Composer".into(), "TPUB" => "Publisher".into(), "TCOP" => "Copyright".into(),
            "COMM" => "Comment".into(), "TPOS" => "Disc".into(), "TBPM" => "BPM".into(),
            _ => k.into() }
    };
    ...
    match action {
        "info" | "metadata" => {
            ...
            if let Some(ref f) = frame {
                let ver = match f.mpeg_ver { 1 => "MPEG-1", 2 => "MPEG-2", _ => "MPEG-2.5" };
                ...
                out.push(format!("Channels:  {}", if f.channels == 1 { "Mono" } else { "Stereo" }));
            }
            if let Some(ref t) = id3v2 {
                out.push(format!("ID3v2: v2.{}.{}", t.version, t.revision));
                if t.flags & 0x40 != 0 { out.push("  Extended header present".to_string()); }
                if t.flags & 0x10 != 0 { out.push("  Footer present".to_string()); }
            }
            if !id3v1.is_empty() { out.push("ID3v1: present".to_string()); }
            for (k, v) in &tags { out.push(format!("  {}: {}", tag_label(k), v)); }
            out.push(format!("File size: {}", human_size(file_size)));
            out.join("\n")
        }
        "tags" => {
            if tags.is_empty() { return "No ID3 tags found.".to_string(); }
            tags.iter().map(|(k, v)| format!("{}: {}", tag_label(k), v)).collect::<Vec<_>>().join("\n")
        }
        "validate" => {
            let mut issues = vec![];
            if !has_id3v2 && id3v1.is_empty() { issues.push("No ID3 tags found".to_string()); }
            if frame.is_none() { issues.push("No valid MPEG frame header found".to_string()); }
            if tags.iter().all(|(k, _)| k != "TIT2") { issues.push("Missing title tag (TIT2)".to_string()); }
            if tags.iter().all(|(k, _)| k != "TPE1") { issues.push("Missing artist tag (TPE1)".to_string()); }
            if issues.is_empty() { "VALID — MP3 structure and tags look well-formed.".to_string() }
            else { format!("WARNINGS:\n{}", issues.iter().map(|s| format!("  • {}", s)).collect::<Vec<_>>().join("\n")) }
        }
...
fn parse_flac(b: &[u8]) -> Option<FlacInfo> {
    if b.len() < 42 || &b[0..4] != b"fLaC" { return None; }
    let mut pos = 4usize;
    ...
    loop {
        ...
        match block_type {
            0 => { // STREAMINFO — 34 bytes
                if block_len >= 34 {
                    ...
                    info = Some(FlacInfo {
                        ...
                        sample_rate: sr, channels, bits_per_sample: bps,
                        total_samples: ts, file_size: b.len() as u64,
                        vorbis_comments: vec![],
                    });
                }
            }
            4 => { // VORBIS_COMMENT
                ...
                let mut cp = 0usize;
                if cp + 4 > d.len() { break; }
                let vendor_len = ru32le(d, cp) as usize; cp += 4;
                if cp + vendor_len > d.len() { break; }
                cp += vendor_len;
                if cp + 4 > d.len() { break; }
                let num_comments = ru32le(d, cp) as usize; cp += 4;
                for _ in 0..num_comments {
                    if cp + 4 > d.len() { break; }
                    let clen = ru32le(d, cp) as usize; cp += 4;
                    if cp + clen > d.len() { break; }
...
fn dispatch_flac(action: &str, b: &[u8]) -> String {
    ...
    let duration = if m.sample_rate > 0 { m.total_samples as f64 / m.sample_rate as f64 } else { 0.0 };
    ...
    match action {
        "info" | "metadata" => {
            ...
            if !m.vorbis_comments.is_empty() {
                ...
                for (k, v) in &m.vorbis_comments { out.push(format!("  {}: {}", k, v)); }
            }
            out.join("\n")
        }
        "tags" => {
            if m.vorbis_comments.is_empty() { return "No Vorbis comments found.".to_string(); }
            m.vorbis_comments.iter().map(|(k, v)| format!("{}: {}", k, v)).collect::<Vec<_>>().join("\n")
        }
        "validate" => {
            let mut issues = vec![];
            if m.sample_rate == 0 { issues.push("Sample rate is zero".to_string()); }
            if m.channels == 0 { issues.push("Channel count is zero".to_string()); }
            if m.bits_per_sample == 0 { issues.push("Bit depth is zero".to_string()); }
            if m.total_samples == 0 { issues.push("Total samples is zero (or unknown)".to_string()); }
            if m.vorbis_comments.iter().all(|(k, _)| k != "TITLE") { issues.push("Missing TITLE tag".to_string()); }
            if issues.is_empty() { "VALID — FLAC file structure is well-formed.".to_string() }
            else { format!("WARNINGS:\n{}", issues.iter().map(|s| format!("  • {}", s)).collect::<Vec<_>>().join("\n")) }
        }
...
fn parse_ogg(b: &[u8]) -> Option<OggMeta> {
    if b.len() < 27 || &b[0..4] != b"OggS" { return None; }
    let mut meta = OggMeta {
        serial: ru32le(b, 14),
        page_count: 0,
        has_vorbis: false, has_opus: false, has_theora: false,
        sample_rate: 0, channels: 0, bitrate_nominal: 0,
        comments: vec![], file_size: b.len() as u64,
    };
    let mut pos = 0usize;
    while pos + 27 <= b.len() {
        ...
        if payload.len() >= 7 && &payload[1..7] == b"vorbis" {
            ...
            if payload[0] == 1 && payload.len() >= 30 {
                ...
                meta.bitrate_nominal = i32::from_le_bytes([payload[20], payload[21], payload[22], payload[23]]);
...
fn parse_vorbis_comment(data: &[u8], start: usize, out: &mut Vec<(String, String)>) {
    ...
    let vlen = ru32le(data, p) as usize; p += 4;
    if p + vlen > data.len() { return; }
    p += vlen;
    if p + 4 > data.len() { return; }
    let nc = ru32le(data, p) as usize; p += 4;
    for _ in 0..nc {
        if p + 4 > data.len() { break; }
        let cl = ru32le(data, p) as usize; p += 4;
        if p + cl > data.len() { break; }
...
fn dispatch_ogg(action: &str, b: &[u8]) -> String {
    ...
    let codec = if m.has_opus { "Ogg Opus" } else if m.has_vorbis { "Ogg Vorbis" } else if m.has_theora { "Ogg Theora" } else { "Ogg (unknown codec)" };
    ...
    match action {
        "info" | "metadata" => {
            ...
            if m.channels > 0 { out.push(format!("Channels:        {}", m.channels)); }
            if m.sample_rate > 0 { out.push(format!("Sample rate:     {} Hz", m.sample_rate)); }
            if m.has_vorbis && m.bitrate_nominal > 0 {
                out.push(format!("Nominal bitrate: {} kbps", m.bitrate_nominal / 1000));
            }
            if !m.comments.is_empty() {
                out.push("Vorbis comments:".to_string());
                for (k, v) in &m.comments { out.push(format!("  {}: {}", k, v)); }
            }
            out.join("\n")
        }
        "tags" => {
            if m.comments.is_empty() { return "No Vorbis comments found.".to_string(); }
            m.comments.iter().map(|(k, v)| format!("{}: {}", k, v)).collect::<Vec<_>>().join("\n")
        }
        "validate" => {
            let mut issues = vec![];
            ...
            if m.channels == 0 { issues.push("Channel count is zero".to_string()); }
            if m.has_vorbis && m.sample_rate == 0 { issues.push("Sample rate is zero".to_string()); }
            if m.comments.iter().all(|(k, _)| k != "TITLE") { issues.push("Missing TITLE tag".to_string()); }
            if issues.is_empty() { "VALID — Ogg file structure looks well-formed.".to_string() }
            else { format!("WARNINGS:\n{}", issues.iter().map(|s| format!("  • {}", s)).collect::<Vec<_>>().join("\n")) }
        }
...
enum AudioFormat { Wav, Mp3, Flac, Ogg, Unknown }

fn detect_format(b: &[u8]) -> AudioFormat {
    if b.len() < 4 { return AudioFormat::Unknown; }
    if b.len() >= 12 && &b[0..4] == b"RIFF" && &b[8..12] == b"WAVE" { return AudioFormat::Wav; }
    if &b[0..3] == b"ID3" { return AudioFormat::Mp3; }
    if b.len() >= 4 && &b[0..4] == b"fLaC" { return AudioFormat::Flac; }
    if &b[0..4] == b"OggS" { return AudioFormat::Ogg; }
    // raw MPEG sync
    if b.len() >= 2 && b[0] == 0xFF && (b[1] & 0xE0) == 0xE0 { return AudioFormat::Mp3; }
    AudioFormat::Unknown
}
