...
fn parse_png(b: &[u8]) -> PngMeta {
    ...
    while pos + 12 <= b.len() {
        ...
        match tag {
            ...
            b"PLTE" => {
                m.palette_colors = Some(len as u32 / 3);
            }
            b"tRNS" => {
            ...
            b"gAMA" if d.len() >= 4 => {
                m.gamma = Some(ru32be(d, 0) as f64 / 100_000.0);
            }
            ...
            b"iTXt" => {
                if let Some(nul) = d.iter().position(|&x| x == 0) {
                    ...
                    if p < d.len() {
                        let v: String =
                            String::from_utf8_lossy(&d[p..]).chars().take(120).collect();
                        if !v.is_empty() {
                            m.text_tags.push((k, v));
                        }
                    }
...
fn dispatch_png(action: &str, b: &[u8]) -> String {
    ...
    match action {
        ...
        "color" => {
            let mut out = format!(
                "Color Mode:  {} ({} bpp)\n",
                png_color_label(m.color_type),
                png_bpp(m.color_type, m.bit_depth)
            );
            out += &format!("Bit Depth:   {} bits/sample\n", m.bit_depth);
            out += &format!(
                "Alpha:       {}\n",
                if png_has_alpha(m.color_type, m.has_transparency) {
                    "Yes"
                } else {
                    "No"
                }
            );
            if let Some(n) = m.palette_colors {
                out += &format!("Palette:     {} colors\n", n);
            }
            if m.has_srgb {
                out += "Color Space: sRGB\n";
            }
            if m.has_icc {
                out += "ICC Profile: Present\n";
            }
        ...
        "validate" => {
            ...
            if !m.valid_ihdr {
                issues.push("IHDR chunk missing or corrupt".to_string());
            }
            if m.width == 0 || m.height == 0 {
                issues.push("Zero dimension".to_string());
            }
            if m.color_type == 3 && m.palette_colors.is_none() {
                issues.push("Indexed PNG missing PLTE chunk".to_string());
            }
            if issues.is_empty() {
                format!(
        ...
        _ => {
            ...
            if let (Some(xd), Some(yd)) = (m.x_dpi, m.y_dpi) {
                if (xd - yd).abs() < 1.0 {
                    out += &format!("Resolution: {:.0} DPI\n", xd);
                } else {
                    out += &format!("Resolution: {:.0} × {:.0} DPI\n", xd, yd);
                }
            }
            out += &format!(
                "Interlaced: {}\n",
            ...
            if m.animated {
                out += &format!("Animated:   Yes ({} frames, APNG)\n", m.frame_count);
            }
            if m.has_icc {
                out += "ICC Profile: Present\n";
            }
            ...
            if !m.text_tags.is_empty() {
                out += &format!("Text Tags:  {} embedded\n", m.text_tags.len());
            }
            out
        }
    }
...
fn parse_jpeg(b: &[u8]) -> JpegMeta {
    ...
    while pos + 2 <= b.len() {
        ...
        if seg_len < 2 || pos + seg_len > b.len() {
            break;
        }
        ...
        match marker {
            0xE0 if d.len() >= 9 && &d[..5] == b"JFIF\0" => {
                ...
                m.density_unit = d[7];
                if d.len() >= 12 {
                    m.x_density = ru16be(d, 8);
                    m.y_density = ru16be(d, 10);
                }
            }
            0xE1 => {
                if d.len() >= 6 && &d[..6] == b"Exif\0\0" {
                    m.has_exif = true;
                } else if d.len() >= 29 && &d[..29] == b"http://ns.adobe.com/xap/1.0/\0" {
                    m.has_xmp = true;
                }
            }
            0xE2 if d.len() >= 12 && &d[..12] == b"ICC_PROFILE\0" => {
                m.has_icc = true;
            }
            0xFE => {
                m.comment = Some(String::from_utf8_lossy(d).chars().take(200).collect());
            }
            0xC0 => {
    ...
    match action {
        "dimensions" => {
            let mut out = format!(
                ...
                m.width,
                m.height,
                aspect(m.width as u32, m.height as u32)
            );
            if m.x_density > 0 {
                out += &format!(
                    "DPI:    {}\n",
        ...
        "metadata" => {
            let mut out = String::new();
            if m.has_exif {
                out += "EXIF:        Present (use exif_tools for detail)\n";
            }
            if m.has_xmp {
                out += "XMP:         Present\n";
            }
            ...
            if out.is_empty() {
                out = "No embedded metadata found.\n".to_string();
            }
            out
        }
        "validate" => {
    ...
    while pos < b.len() {
        match b[pos] {
            ...
            0x2C => {
                ...
                pos += 10;
                if has_lct {
                    pos += 3 * 2usize.pow(lct_sz as u32 + 1);
                }
...
fn dispatch_bmp(action: &str, b: &[u8]) -> String {
    ...
    match action {
        ...
        "validate" => {
            ...
            if m.header_size < 40 {
                issues.push(format!("Non-standard header size {}", m.header_size));
            }
            if m.width == 0 || abs_h == 0 {
                issues.push("Zero dimensions".to_string());
            }
            if issues.is_empty() {
                format!("VALID\n  ✓ BITMAPINFOHEADER, {}×{}\n", m.width, abs_h)
            } else {
