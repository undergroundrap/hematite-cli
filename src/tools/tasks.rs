...
fn list_tasks(path: &PathBuf) -> Result<String, String> {
    ...
    Ok(format!("--- TASK LEDGER (.hematite/TASK.md) ---\n\n{}", content))
}
...
fn update_task(path: &PathBuf, id: usize, status: &str) -> Result<String, String> {
    if !path.exists() { return Err("Task ledger not found".into()); }
    let content = fs::read_to_string(path).map_err(|e| e.to_string())?;
    let mut lines: Vec<String> = content.lines().map(|s| s.to_string()).collect();

    if id < 1 || id > lines.len() {
        return Err(format!("Invalid task ID {id}. Ledger has {} items.", lines.len()));
    }

    let line = &mut lines[id - 1];
    if line.starts_with("- [") && line.len() >= 5 {
        // Update the [ ] status (index 3)
        let new_line = format!("- [{}] {}", status.trim_matches(|c| c == '[' || c == ']' || c == ' '), &line[6..]);
        *line = new_line;
    } else {
