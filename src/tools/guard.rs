...
pub const PROTECTED_FILES: &[&str] = &[
    // Windows System
    "C:\\Windows", "C:\\Program Files", "C:\\$Recycle.Bin", 
    "System Volume Information", "C:\\Users\\Default",
    // Linux/Unix System
    "/etc", "/dev", "/proc", "/sys", "/root", "/var/log", "/boot",
    // User Sensitives
    ".bashrc", ".zshrc", ".bash_history", ".gitconfig", ".ssh/", 
    ".aws/", ".env", "credentials.json", "auth.json", "id_rsa",
    // Hematite Internal
    ".mcp.json", "hematite_memory.db"
];
...
pub fn path_is_safe(workspace_root: &Path, target: &Path) -> Result<PathBuf, String> {
    ...
    target_str = target_str.replace("\\", "/").replace("\u{005c}", "/").replace("%5c", "/");
    ...
    for protected in PROTECTED_FILES {
        ...
        if target_str.contains(&prot_lower) {
            return Err(format!("AccessDenied: Path {} hits the Hematite Security Blacklist natively: {}", target_str, protected));
        }
    }
    ...
    let resolved_str = resolved_path.to_string_lossy().to_string().to_lowercase().replace("\\", "/");
    for protected in PROTECTED_FILES {
        let prot_lower = protected.to_lowercase().replace("\\", "/");
        if resolved_str.contains(&prot_lower) {
            return Err(format!("AccessDenied: Canonicalized Sandbox resolution natively hits Blacklist bounds: {}", protected));
        }
    }
...
pub fn bash_is_safe(cmd: &str) -> Result<(), String> {
    let lower = cmd.to_lowercase().replace("\\", "/").replace("\u{005c}", "/").replace("%5c", "/");
    for protected in PROTECTED_FILES {
        let prot_lower = protected.to_lowercase().replace("\\", "/");
...
pub fn classify_bash_risk(cmd: &str) -> RiskLevel {
    ...
    let high = [
        // File destruction
        "rm -", "rm /", "del /", "del /f", "rmdir /s", "remove-item -r",
        ...
        // System
        "shutdown", "restart-computer", "taskkill", "format-volume",
        "diskpart", "format c", "del c:\\",
        // Secrets
        ".ssh/", ".aws/", "credentials.json",
    ];
    ...
    let safe_prefixes = [
        "cargo check", "cargo build", "cargo test", "cargo fmt",
        "cargo clippy", "cargo run", "cargo doc", "cargo tree",
        "rustc ", "rustfmt ",
...
mod tests {
    ...
    fn test_blacklist_windows_system() {
        ...
        assert!(result.is_err(), "Windows System directory should be blocked!");
    ...
    fn test_bash_blacklist() {
        ...
        assert!(result.is_err(), "Bash command touching Windows should be blocked!");
