...
fn parse_unix_services(status_text: &str, startup_text: &str) -> Vec<ServiceEntry> {
    ...
    for line in status_text.lines() {
        ...
        let Some(load) = it.next() else {
            continue;
        };
        let Some(active) = it.next() else {
            continue;
        };
...
"#;
        ...
        if text == "NONE" {
            ...
            for line in text.lines() {
                let mut it = line.splitn(4, '|');
                if let (Some(name), Some(desc), Some(status)) = (it.next(), it.next(), it.next()) {
...
"#;
        ...
        if !radio_problems.is_empty() || !device_problems.is_empty() {
            let problem_labels = {
                let mut s = String::new();
                for (i, device) in radio_problems
                    .iter()
...
"#
        ...
        if let Some(id) = event_id {
            let _ = write!(query_desc, ", Event ID {id}");
        }
        if let Some(src) = source {
            let _ = write!(query_desc, ", source '{src}'");
        }
        if log != "*" {
            let _ = write!(query_desc, ", log '{log}'");
        }
        if let Some(l) = level {
            let _ = write!(query_desc, ", level '{l}'");
        }
