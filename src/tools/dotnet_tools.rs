...
pub async fn execute(args: &Value) -> Result<String, String> {
    ...
    match action {
        ...
        other => Err(format!("Unknown action '{other}'. Choose: info, packages, targets, references, validate")),
    }
}
...
fn load_text(args: &Value) -> Result<(String, String), String> {
    if let Some(f) = args.get("file").and_then(|v| v.as_str()) {
        let content = std::fs::read_to_string(f)
            .map_err(|e| format!("Cannot read '{}': {e}", f))?;
        let kind = detect_kind(f);
        return Ok((content, kind));
    }
    let text = args.get("text").or_else(|| args.get("xml"))
        .and_then(|v| v.as_str())
...
fn detect_kind(path: &str) -> String {
    ...
    if p.ends_with(".sln")   { "sln".into() }
    else if p.ends_with(".fsproj") { "fsproj".into() }
    else if p.ends_with(".vbproj") { "vbproj".into() }
    else { "csproj".into() }
}
...
fn item_group_items<'a>(xml: &'a str, tag: &str) -> Vec<&'a str> {
    ...
    let mut out = Vec::new();
    let mut rest = xml;
    while let Some(pos) = rest.find(&open).or_else(|| rest.find(&open2)).or_else(|| rest.find(&open3)) {
        let slice = &rest[pos..];
...
fn parse_sln(text: &str) -> Vec<SlnProject> {
    ...
    for line in text.lines() {
        let t = line.trim();
        if !t.starts_with("Project(") { continue; }
        // Project("{TYPE-GUID}") = "Name", "Path", "{PROJ-GUID}"
        let parts: Vec<&str> = t.splitn(2, '=').collect();
        if parts.len() < 2 { continue; }
        let lhs = parts[0].trim();
        let rhs = parts[1].trim();
        let type_guid = lhs.trim_start_matches("Project(\"")
            .trim_end_matches("\")")
        ...
        let guid = fields[2].trim().trim_matches('"')
            .trim_matches('{').trim_matches('}').to_string();
        projects.push(SlnProject { type_guid, name, path, guid });
    }
    projects
}
...
fn do_info(args: &Value) -> Result<String, String> {
    ...
    if kind == "sln" {
        ...
        let format_line = text.lines()
            .find(|l| l.trim_start().starts_with("Microsoft Visual Studio Solution File"))
        ...
        let real_count = projects.iter().filter(|p| p.type_guid.to_uppercase() != "2150E333-8FDC-42A3-9474-1A3956D46DE8").count();
        ...
        let ext = if kind == "fsproj" { "F#" } else if kind == "vbproj" { "VB.NET" } else { "C#" };

        out.push_str(&format!("{ext} Project Info\n"));
        out.push_str(&"─".repeat(40));
        out.push('\n');
        if !sdk.is_empty()       { out.push_str(&format!("SDK              : {sdk}\n")); }
        if !target_fw.is_empty() { out.push_str(&format!("Target Framework : {target_fw}\n")); }
        if !output.is_empty()    { out.push_str(&format!("Output Type      : {output}\n")); }
        if !assembly.is_empty()  { out.push_str(&format!("Assembly Name    : {assembly}\n")); }
        if !root_ns.is_empty()   { out.push_str(&format!("Root Namespace   : {root_ns}\n")); }
        if !version.is_empty()   { out.push_str(&format!("Version          : {version}\n")); }
        if !nullable.is_empty()  { out.push_str(&format!("Nullable         : {nullable}\n")); }
        if !lang_ver.is_empty()  { out.push_str(&format!("LangVersion      : {lang_ver}\n")); }
        ...
        if !defines.is_empty() { out.push_str(&format!("Define Constants : {defines}\n")); }

        let gen_doc = tag_first(&text, "GenerateDocumentationFile").unwrap_or_default();
        if gen_doc.to_lowercase() == "true" { out.push_str("Documentation    : enabled\n"); }

        let treat_warn = tag_first(&text, "TreatWarningsAsErrors").unwrap_or_default();
        if treat_warn.to_lowercase() == "true" { out.push_str("Warnings         : treated as errors\n"); }

        let publish = tag_first(&text, "PublishSingleFile").unwrap_or_default();
        if publish.to_lowercase() == "true" { out.push_str("Publish          : single-file\n"); }

        let self_contained = tag_first(&text, "SelfContained").unwrap_or_default();
        if self_contained.to_lowercase() == "true" { out.push_str("Self-contained   : true\n"); }

        let invariant = tag_first(&text, "InvariantGlobalization").unwrap_or_default();
        if invariant.to_lowercase() == "true" { out.push_str("Globalization    : invariant\n"); }
    }
...
fn do_packages(args: &Value) -> Result<String, String> {
    ...
    out.push('\n');
    out.push_str(&format!("{:<w_name$}  {:<w_ver$}  Condition\n", "Package", "Version"));
    out.push_str(&format!("{:<w_name$}  {:<w_ver$}  ─────────\n", "─".repeat(w_name), "─".repeat(w_ver)));
    ...
    let wildcards: Vec<&str> = rows.iter()
    ...
    while let Some(pos) = rest.find("<PropertyGroup") {
        ...
        match (cond_start, close_tag) {
            (Some(cs), Some(ct)) if cs < ct => {
                ...
                let end_q = val_rest.find('"').or_else(|| val_rest.find('\'')).unwrap_or(val_rest.len());
...
fn do_sln_targets(text: &str) -> Result<String, String> {
    let projects = parse_sln(text);
    let buildable: Vec<&SlnProject> = projects.iter()
...
fn extract_simple_props(xml: &str) -> Vec<(String, String)> {
    ...
    while let Some(pos) = rest.find('<') {
        ...
        let name_end = slice.find(|c: char| c == '>' || c == ' ' || c == '/').unwrap_or(slice.len());
        let tag = &slice[..name_end];
        if !tag.is_empty() && !tag.starts_with('/') && !tag.starts_with('!') && !skip.iter().any(|s| *s == tag) {
            let close = format!("</{tag}>");
        ...
        for item in &framework_refs {
            ...
            let hint = {
                ...
                if let Some(pos) = text.find(&search) {
                    let after = &text[pos..];
                    tag_values(after, "HintPath").into_iter().next().unwrap_or_default()
                } else {
                    String::new()
                }
            };
...
fn do_validate(args: &Value) -> Result<String, String> {
    ...
    if kind == "sln" {
        let projects = parse_sln(&text);
        let has_format = text.lines().any(|l| l.contains("Microsoft Visual Studio Solution File"));
        if !has_format { issues.push("Missing 'Microsoft Visual Studio Solution File' header line".into()); }
        if projects.is_empty() { issues.push("No Project entries found".into()); }
        let real: Vec<&SlnProject> = projects.iter()
            .filter(|p| p.type_guid.to_uppercase() != "2150E333-8FDC-42A3-9474-1A3956D46DE8")
            .collect();
        if real.is_empty() { issues.push("No buildable (non-folder) projects".into()); }
        // Check for duplicate GUIDs
        let mut guids: Vec<&str> = projects.iter().map(|p| p.guid.as_str()).collect();
        guids.sort();
        let mut prev = "";
        for g in &guids {
            if *g == prev { issues.push(format!("Duplicate project GUID: {g}")); }
            prev = g;
        }
        // Check for relative .sln path on Windows (backslash expected)
        for p in &real {
            if p.path.contains('/') && !p.path.starts_with("http") {
                issues.push(format!("'{}' uses forward slashes — expect backslashes on Windows", p.path));
            }
        }
    } else {
        let sdk = tag_attrs(&text, "Project")
        ...
        let fw = tag_first(&text, "TargetFramework").or_else(|| tag_first(&text, "TargetFrameworks"));
        if fw.is_none() { issues.push("No <TargetFramework> or <TargetFrameworks> specified".into()); }
        ...
        for item in &items {
            ...
            if ver.is_empty() {
                issues.push(format!("PackageReference '{name}' has no Version attribute"));
            }
        }

        // Hardcoded connection strings / secrets in PropertyGroups
        let lower = text.to_lowercase();
        if lower.contains("password=") || lower.contains("connectionstring") || lower.contains("api_key") || lower.contains("apikey") {
        ...
        if tag_first(&text, "TargetFramework").is_some() && tag_first(&text, "TargetFrameworks").is_some() {
            issues.push("Both <TargetFramework> and <TargetFrameworks> defined — use only one".into());
        }
    }
