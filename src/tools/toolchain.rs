...
- `inspect_host`\n\
  Good for: structured read-only inspection of the current machine such as common developer tool versions, PATH analysis, process snapshots, desktop items, Downloads summaries, listening ports, repo-doctor checks, and arbitrary directory or disk-size reports.\n\
...
- Use `inspect_host` before `shell` for read-only questions about PATH, installed tools, running processes, desktop items, Downloads size, listening ports, repo-health summaries, or directory/disk summaries.\n\
...
1. `inspect_host`\n\
   ...
   Use: start with the closest topic such as `summary`, `toolchains`, `path`, `processes`, `desktop`, `downloads`, `ports`, `repo_doctor`, `directory`, or `disk`.\n\
2. `shell`\n\
