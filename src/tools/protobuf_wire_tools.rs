        ...
        match wire_type {
            ...
            2 => {
                ...
                fields.push(Field {
                    field_num,
                    wire_type,
                    value: FieldValue::LenDelim(bytes),
                });
            }
            5 => {
                if pos + 4 > data.len() {
        ...
        match &f.value {
            ...
            FieldValue::LenDelim(bytes) => {
                if bytes.is_empty() {
                    ...
                    let display: String = preview
                        .chars()
                        .map(|c| if c.is_control() && c != '\n' { '·' } else { c })
                        .collect();
                    out.push_str(&format!(
                        "{}field {:3}  [LenDelim] {:3} bytes  string: \"{}\"\n",
                        pad,
                        f.field_num,
                        bytes.len(),
                        display
                    ...
                    out.push_str(&format!(
                        "{}field {:3}  [LenDelim] {:3} bytes  nested message:\n",
                        pad,
                        f.field_num,
                        bytes.len()
                    ));
...
fn action_fields(data: &[u8]) -> Result<String, String> {
    ...
    if fields.is_empty() {
        return Err(
            "No protobuf fields decoded — check that input is valid protobuf wire format".into(),
        );
    }
    let mut out = String::new();
    out.push_str(&format!(
...
fn action_explain(data: &[u8]) -> Result<String, String> {
    ...
    let fields = decode_fields(data);
    if fields.is_empty() {
        return Err("No protobuf fields decoded".into());
    }

    let mut out = String::new();
    out.push_str(&format!(
        "── Protobuf Wire Explain ({} bytes, {} fields) ──────────\n\n",
        data.len(),
        fields.len()
    ));
...
pub async fn execute(args: &Value) -> Result<String, String> {
    ...
    match action {
        ...
        other => Err(format!(
            "Unknown action '{}'. Use: decode, fields, strings, explain",
            other
        )),
    }
}
