...
For structured workstation and network questions, prefer `inspect_host` first. It covers 115+ read-only topics across the full SysAdmin, Network Admin, security, and developer tooling stack:
...
The current release train successfully validates the **115+-topic Diagnostic Matrix**. This finding proves that a single local agent can maintain high-fidelity parity across Windows (WMI/NetAPI) and Unix (sysfs/journald) without requiring external cloud-based diagnostic APIs.
