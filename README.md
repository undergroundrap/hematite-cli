...
- **SysAdmin + Network Admin built in**: 73 read-only inspection topics cover CPU/RAM load, processes, services, ports, storage, hardware DNA, security posture, crash history, scheduled tasks, Group Policy, certificates, system integrity, domain identity, connectivity, Wi-Fi, active connections, VPN state, proxy config, firewall rules, traceroute, DNS cache, ARP table, and routing table — plus high-fidelity Tier 3 tools for NTFS permissions, login history, network share accessibility, and registry persistence auditing. All grounded, no guessing.
...
- **Self-Correcting Runtime Guards**: automatically catches and blocks raw shell commands for any inspectable topic, redirecting them to the right structured tool (e.g., `Get-Acl` -> `permissions`, `Get-EventLog` -> `login_history`) and retrying the turn without interrupting you.
