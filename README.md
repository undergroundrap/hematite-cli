...
- **Integrated host inspection suite**: 78+ read-only diagnostic topics cover CPU/RAM load, active processes, services, listening ports, block storage, hardware specifications, security configuration, event-log history, scheduled tasks, Group Policy, certificate lifecycle, system integrity, and domain membership — plus high-fidelity tools for NTFS/ACL permissions, authentication logs, UNC share accessibility, and registry persistence auditing. Now includes diagnostic topics for **CPU Thermal Throttling, Windows Activation status, and Patch (KB) history**.
...
- **Natural-language SysAdmin + Network Admin** — 78+ read-only inspection topics cover full workstation health (telemetry, processes, hardware specs, security, event history, GPO, certificates, integrity) and complete network state (connectivity, Wi-Fi, active connections, VPN, proxy, firewall rules, traceroute, DNS cache, ARP/routing tables) — all derived from real tool output, not model guesses.
